...
Movelist toMovelist(const vector<Move>& vec) {
    Movelist moves;
...
vector<Move> toVector(const Movelist& moves) {
    vector<Move> vec;
...
public:
    Board board;
    // Use board hash as key for caching
    struct CacheEntry {
        vector<Move> cached_moves;
        float utility_value;
        bool utility_computed;
        bool moves_cached;
        
        CacheEntry() : utility_computed(false), moves_cached(false) {}
    };
    ...
    float utility(Board board, float wheights[], int depth) {
        uint64_t board_hash = board.hash();
        // See if utility is cached
        if (move_cache.find(board_hash) != move_cache.end() && move_cache[board_hash].utility_computed) {
            return move_cache[board_hash].utility_value;
        }
        Movelist moves;
        Movelist fake_moves;
        // See if moves are cached
        if (move_cache.find(board_hash) != move_cache.end() && move_cache[board_hash].moves_cached) {
            moves = toMovelist(move_cache[board_hash].cached_moves);
        }
        else {
            movegen::legalmoves(moves, board);
            if (true) {
                CacheEntry& entry = move_cache[board_hash];
                entry.moves_cached = true;
                entry.cached_moves = toVector(moves);
            }
        }
        ...
        // Cache computed utility
        if (true) {
            CacheEntry& entry = move_cache[board_hash];
            entry.utility_value = utility;
            entry.utility_computed = true;
        }
        // Return utility
        return utility;
    }
    ...
    pair<Move, float> alphaBeta(Board& board, float alpha, float beta, int depth, Color player, float wheights[]) {
        ...
        // Generate all moves
        Movelist moves;
        // Check if moves were cached
        if (move_cache.find(board_hash) != move_cache.end() && move_cache[board_hash].moves_cached) {
            moves = toMovelist(move_cache[board_hash].cached_moves);
            move_cache.erase(board_hash);
        }
        else {
            movegen::legalmoves(moves, board);
            CacheEntry& entry = move_cache[board_hash];
        ...
        for (auto m : moves) {
            board.makeMove(m);
            float score = utility(board, wheights, depth-1);
            board.unmakeMove(m);
            scored.emplace_back(m, score);
        }
        // Sort descending if White to move , ascending if Black 
...
int main(int argc, char* argv[]) {
    ...
    // Think about how to use class efficiently, I mean maybe use values you know from before fen
    ...
    if (argc == 5) {
        ...
        while  (fen!="0") {
            ...
            auto my_move_pair = my_solver.alphaBeta(my_solver.board, alpha, beta, depth, my_solver.board.sideToMove(), wheights);
